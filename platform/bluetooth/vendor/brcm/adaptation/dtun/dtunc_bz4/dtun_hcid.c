...
// SecFeature.SPRINT by jheeya.kim static int char_len;
                             ...
                             int               len)
{
  ...
  while (p < end)
    {
      int i, mask, char_len; // SecFeature.SPRINT by jheeya.kim 
      unsigned int result;
...
*/
    PRINTFUNC(); // SecFeature.SPRINT by jheeya.kim 
...
void dtun_dm_sig_authorize_req(tDTUN_DEVICE_SIGNAL *msg)
{
	int err;
	DBusError derr;
	uint32_t cur_cod; // SecFeature.SPRINT by jheeya.kim
        PRINTFUNC();
...
*/
	info( "current cod = %x, received cod = %x", cur_cod, msg->authorize_req.info.cod ); // SecFeature.SPRINT by jheeya.kim
...
static void dtun_am_sig_av_event(tDTUN_DEVICE_SIGNAL *msg)
{
    ...
    if (msg->av_event.info.event == 2) /* BTA_AV_OPEN_EVT */
    {
        ...
        if (orig_msg && err_msg[0]) // SecFeature.SPRINT by jheeya.kim
        // SecFeature.SPRINT by jheeya.kim if(err_msg[0] != 0)
        {
