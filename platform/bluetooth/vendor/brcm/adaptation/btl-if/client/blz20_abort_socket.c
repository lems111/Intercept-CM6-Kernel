...
#define LOG_TAG "ASOCKWRP"

//#ifdef BRCM_BT_USE_BTL_IF
/* map all blz socket calls to btl-if */
...
int asocket_connect(struct asocket *s, const struct sockaddr *addr,
    ...
    if (ret && errno == EINPROGRESS) {
        ...
        pfd[0].fd = s->fd;
        pfd[0].events = POLLOUT | POLLRDHUP;
        pfd[0].revents = 0;
        pfd[1].fd = s->abort_fd[0];
        pfd[1].events = POLLIN | POLLRDHUP; //<<< POLLRDHUP added for socket closing event
        pfd[1].revents = 0;
...
void asocket_abort(struct asocket *s) {
    ...
    LOGV("%s [%d,%d,%d]", __FUNCTION__, s->fd, s->abort_fd[0], s->abort_fd[1]);
