...
BOOLEAN wrp_close_s_only(t_wsock *ws, int s)
{
    ...
    /*
    debug("Henry : Check active AG socket");

    for(i = 2; i < SOCKET_USER_MAX; i++)
    {
        ws_brcm = &wcb[SUB_AG].ws[i];

        if((ws_brcm->wsock != DATA_SOCKET_INVALID) && (ws_brcm->busy == 1))
        {
            wrp_close_full(ws_brcm);
        }
    }
    */    
