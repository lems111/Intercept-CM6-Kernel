...
struct btd_adapter {
	...
	uint8_t txcarrier_carrier;
	uint8_t txcarrier_frequency;
	uint8_t txcarrier_mode;
	uint8_t txcarrier_modulation;
	uint8_t txcarrier_txpower;
	uint8_t txcarrier_txpower_dbm;
	uint8_t txcarrier_txpower_index;

	gint ref;
};

static DBusMessage *send_SetTxCarrierFrequency(DBusConnection *conn,
                                 DBusMessage *msg,
                                 uint32_t mode,
                                 void *data);
...
static DBusMessage *set_TXCarrier_carrier(DBusConnection *conn, DBusMessage *msg,
			     uint32_t val, void *data)
{
	struct btd_adapter *adapter = (struct btd_adapter *)data;

	adapter->txcarrier_carrier = val;
	return dbus_message_new_method_return(msg);
}
static DBusMessage *set_TXCarrier_frequency(DBusConnection *conn, DBusMessage *msg,
			       uint32_t val, void *data)
{
	struct btd_adapter *adapter = (struct btd_adapter *)data;

	adapter->txcarrier_frequency = val;
	return dbus_message_new_method_return(msg);
}
static DBusMessage *set_TXCarrier_mode(DBusConnection *conn, DBusMessage *msg,
			  uint32_t val, void *data)
{
	struct btd_adapter *adapter = (struct btd_adapter *)data;

	adapter->txcarrier_mode = val;
	return dbus_message_new_method_return(msg);
}
static DBusMessage *set_TXCarrier_modulation(DBusConnection *conn, DBusMessage *msg,
				uint32_t val, void *data)
{
	struct btd_adapter *adapter = (struct btd_adapter *)data;

	adapter->txcarrier_modulation = val;
	return dbus_message_new_method_return(msg);
}
static DBusMessage *set_TXCarrier_txpower(DBusConnection *conn, DBusMessage *msg,
			      uint32_t val, void *data)
{
	struct btd_adapter *adapter = (struct btd_adapter *)data;

	adapter->txcarrier_txpower = val;
	return dbus_message_new_method_return(msg);
}
static DBusMessage *set_TXCarrier_txpower_dbm(DBusConnection *conn, DBusMessage *msg,
				   uint32_t val, void *data)
{
	struct btd_adapter *adapter = (struct btd_adapter *)data;

	adapter->txcarrier_txpower_dbm = val;
	return dbus_message_new_method_return(msg);
}

static DBusMessage *set_TXCarrier_txpower_index(DBusConnection *conn, DBusMessage *msg,
				   uint32_t val, void *data)
{
	struct btd_adapter *adapter = (struct btd_adapter *)data;

	adapter->txcarrier_txpower_index = val;

	msg = send_SetTxCarrierFrequency(conn, msg, val, data);
                                 ...
                                 void *data)
{
	...
	info("adapter.c:send_SetTxCarrierFrequency(carrier=%d,freq=%d)",
		adapter->txcarrier_carrier, adapter->txcarrier_frequency);

	info("adapter.c:adapter_ops->send_set_tx_carrier_frequency=0x%x)",
		adapter_ops->send_set_tx_carrier_frequency);

	info("adapter.c:before adapter_ops->send_set_tx_carrier_frequency(carrier=%d,freq=%d)",
		adapter->txcarrier_carrier, adapter->txcarrier_frequency);

        err = adapter_ops->send_set_tx_carrier_frequency(adapter->dev_id,
                                                         adapter->txcarrier_carrier,
                                                         adapter->txcarrier_frequency,
                                                         adapter->txcarrier_mode,
                                                         adapter->txcarrier_modulation,
                                                         adapter->txcarrier_txpower,
                                                         adapter->txcarrier_txpower_dbm,
                                                         adapter->txcarrier_txpower_index);

	info("adapter.c:after adapter_ops->send_set_tx_carrier_frequency(carrier=%d,freq=%d), err = %x",
		adapter->txcarrier_carrier, adapter->txcarrier_frequency,err);
					...
					DBusMessage *msg, void *data)
{
	...
	if (g_str_equal("Name", property)) {
	 ...
	 }else if (g_str_equal("TXCarrier_carrier", property)) {
	    uint32_t val;
	    ...
	    return( set_TXCarrier_carrier(conn, msg, val, data) );
          } else if (g_str_equal("TXCarrier_frequency", property)) {
	    uint32_t val;
	    ...
	    return( set_TXCarrier_frequency(conn, msg, val, data) );
          } else if (g_str_equal("TXCarrier_mode", property)) {
	    uint32_t val;
	    ...
	    return( set_TXCarrier_mode(conn, msg, val, data) );
          } else if (g_str_equal("TXCarrier_modulation", property)) {
	    uint32_t val;
	    ...
	    return( set_TXCarrier_modulation(conn, msg, val, data) );
          }else if (g_str_equal("TXCarrier_txpower", property)) {
	    uint32_t val;
	    ...
	    return( set_TXCarrier_txpower(conn, msg, val, data) );
          } else if (g_str_equal("TXCarrier_txpower_dbm", property)) {
	    uint32_t val;
	    ...
	    return( set_TXCarrier_txpower_dbm(conn, msg, val, data) );
          } else if (g_str_equal("TXCarrier_txpower_index", property)) {
	    uint32_t val;
	    ...
	    set_TXCarrier_txpower_index(conn, msg, val, data);
