...
static void sdhci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	switch (host->hwport)
	{
		case 0:
			...
			break;
		case WIFI_PORT_NUM:
#if defined(CONFIG_MACH_QUATTRO)	//cky 20100203
		case WIMAX_PORT_NUM:
#endif
			b_isWlan = 1;
			card_detect = 0;
...
out:
	 ...
	 */
#if defined(CONFIG_MACH_QUATTRO)	//cky 20100203
	if (cardint && (host->hwport == WIFI_PORT_NUM || host->hwport == WIMAX_PORT_NUM))
#else
	if (cardint && host->hwport == WIFI_PORT_NUM)
#endif
	{
...
int sdhci_suspend_host(struct sdhci_host *host, pm_message_t state)
{
	...
	if(host->mmc)
	{
		if(host->hwport != WIFI_PORT_NUM)
		{
#if defined(CONFIG_MACH_QUATTRO)	//cky 20100217 suspend/resume
			if (host->hwport != WIMAX_PORT_NUM)
			{
				ret = mmc_suspend_host(host->mmc, state);
				if (ret)
					return ret;
			...
			{
				if (wimax_suspend) wimax_suspend();
...
#else
			ret = mmc_suspend_host(host->mmc, state);
			printk(KERN_INFO "%s %s CARD(%d) ret(%d)\n", __FUNCTION__, mmc_hostname(host->mmc), card_detect, ret);
			if (ret)
				return ret;
#endif
...
{
	int ret;
	printk("kimhyuns for debugging sdhci_resume_host\n");
	printk("kimhyuns host->hw_name=%s host->hwport=%d, host->mmc->index=%d\n", host->hw_name, host->hwport, host->mmc->index);
