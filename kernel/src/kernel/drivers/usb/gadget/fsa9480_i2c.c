...
void wimax_switch(s32 sel)
{
	DEBUG_FSA9480("[FSA9480] wimax_switch - sel: %d\n", sel);
	if (sel) {    
		gpio_set_value(GPIO_USB_SW_EN_WIMAX, GPIO_LEVEL_LOW);
	}
	else {
		gpio_set_value(GPIO_USB_SW_EN_WIMAX, GPIO_LEVEL_HIGH);
	}
}
#endif
...
void fsa9480_check_usb_connection(void)
{
	...
	DEBUG_FSA9480("[FSA9480] usb_power is %d\n ",usb_power);
#else
...
void usb_switch_mode(int sel)
{
	...
	else if (sel == SWITCH_WIMAX)
	{
		...
		usb_sel(SWITCH_WIMAX);
		fsa9480_write(&fsa9480_i2c_client, REGISTER_MANUALSW1, 0x48); // Audio port (WIMAX)
		wimax_switch(1);
	}
#endif
  else
...
static int fsa9480_codec_probe(struct i2c_adapter *adap, int addr, int kind)
{
	...
	if (pData == 0x24) /* PDA */
		usb_path = SWITCH_PDA;
#if defined(CONFIG_MACH_QUATTRO)
	else if (pData == 0x48) /* WIMAX */
		usb_path = SWITCH_WIMAX;
#endif
	else /* PHONE */
