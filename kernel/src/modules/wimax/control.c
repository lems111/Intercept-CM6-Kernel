...
*/			
		mdelay(100);	//delay for the process release 	
		}
	}
...
VOID controlReceive(MINIPORT_ADAPTER   *Adapter, void *Buffer, unsigned long Length)
{
	...
	ENTER;
#if 0	// rx control frame	
		{
			// dump packets
			UINT i, l = Length;
			u_char *  b = (u_char *)Buffer;
			DumpDebug(READ_REG, "Received packet control [%d] = ", l);
			for (i = 0; i < l; i++) {
...
unsigned int hwSendControl(MINIPORT_ADAPTER *Adapter, void *Buffer, u_long Length)
{
...
#if 0
	{
		...
		DumpDebug(TX_CONTROL, "Sent packet LEN = %ld",Length);
	    DumpDebug(TX_CONTROL, "Sent packet = ");
	    for (i = 0; i < Length; i++) {
		DumpDebug(TX_CONTROL, "%02x", b[i]);
		//if (i != (Length - 1)) DumpDebug(TX_CONTROL, ",");
		//if ((i != 0) && ((i%32) == 0)) DumpDebug(TX_CONTROL, "\n");
	    }
	    //DumpDebug(TX_CONTROL, "\n");
	}
#endif
	Adapter->stats.tx.Control++;
