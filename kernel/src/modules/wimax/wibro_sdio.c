...
extern void gpio_wimax_poweron(void);
extern void gpio_wimax_poweroff(void);
...
//	{
		...
		{		
			u_char cnt=0;
			err = sdio_memcpy_fromio(Adapter->func, Adapter->hw.ReceiveTempBuffer, SDIO_DATA_PORT_REG ,len);
			if(err !=0 || !len )
			{
		...
		remained_len = processSdioReceiveData(Adapter, Adapter->hw.ReceiveTempBuffer, len, 0);
		if(remained_len != 0) DumpDebug(READ_REG, "Should we process for multi packet, Remained Length is = %d",remained_len);
//	}
//    LEAVE;
	return 0;
}
...
uwbrdev_open (struct inode * inode, struct file * file)
{
	...
	DumpDebug(DRV_ENTRY, "open: tgid=%d",current->tgid);
...
uwbrdev_release (struct inode * inode, struct file * file)
{
	...
	ENTER;
	DumpDebug(DISPATCH, "process %d release", current->tgid);
	Adapter = (PMINIPORT_ADAPTER)(file->private_data);
	...
	if(process != NULL) {
		...
		spin_lock_irqsave(&Adapter->ctl.Apps.Lock, flags);
		controlDeleteProcessById(Adapter, current->tgid);
		spin_unlock_irqrestore(&Adapter->ctl.Apps.Lock, flags);
	}
	else { //not found
...
uwbrdev_ioctl (struct inode * inode, struct file * file, u_int cmd, u_long arg)
{
	...
	switch(cmd)
	{
		case CONTROL_IOCTL_WRITE_REQUEST :
		{
			...
			//DumpDebug(DISPATCH, "ioctl: write: process tgid=%d",current->tgid);
...
//    UCHAR stopMessage[4] = {0x1f, 0x2f,0x3f,0x4f};
	...
	//DumpDebug(DRV_ENTRY, "uwbrdev_read: process tgid=%d",current->tgid);
...
//	ENTER;
	if (!Adapter->bHaltPending) {
		...
		if (intrd & SDIO_INT_DATA_READY) {
			if (Adapter_sdio_rx_packet(Adapter) != 0) {
...
void wimax_suspend(void)
{
	...
	gpio_set_value(GPIO_WIMAX_CON1, GPIO_LEVEL_LOW);	// pda active pin low

	// set MEM0 interface
	s3c_gpio_cfgpin(GPIO_WIMAX_RESET_N, S3C_GPIO_SFN(2));

	// set WIMAX_INT
	s3c_gpio_cfgpin(GPIO_WIMAX_INT, S3C_GPIO_SFN(2));	// ext int
	s3c_gpio_setpull(GPIO_WIMAX_INT, S3C_GPIO_PULL_UP);
}
EXPORT_SYMBOL(wimax_suspend);
...
void wimax_resume(void)
{
	...
	gpio_set_value(GPIO_WIMAX_CON1, GPIO_LEVEL_HIGH);	// pda active pin high

	// set OUTPUT
	s3c_gpio_cfgpin(GPIO_WIMAX_RESET_N, S3C_GPIO_SFN(1));

	// clear WIMAX_INT
	s3c_gpio_cfgpin(GPIO_WIMAX_INT, S3C_GPIO_SFN(0));	// input
	s3c_gpio_setpull(GPIO_WIMAX_INT, S3C_GPIO_PULL_NONE);
}
EXPORT_SYMBOL(wimax_resume);
