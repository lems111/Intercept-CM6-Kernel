	...
	if(pDLPacket->MsgType == be16_to_cpu(ETHERTYPE_DL))
	{
		...
		switch (be16_to_cpu(pDLPacket->MsgId)) {
			...
			case MSG_IMAGE_INFO_RESP:				
				...
				}while(!(g_stWiMAXImage.uiOffset == g_stWiMAXImage.uiSize));

				DumpDebug(RX_DPC, "DL_hwIndicatePacket:  MSG_IMAGE_DATA_RESP Image Download Complete\r")));
			...
			case MSG_IMAGE_DATA_RESP:
				...
				break;
			case MSG_RUN_RESP:
				//DumpDebug(FW_DNLD, "DL_hwIndicatePacket:  MSG_RUN_RESP\r");	
				tmpByte = (CHAR *)(pRecvPacket+sizeof(HW_DOWNLOAD_PACKET));
				//DumpDebug(FW_DNLD, "DL_hwIndicatePacket:  MSG_RUN_RESP Value : %x", *tmpByte);

				if(*tmpByte== 0x01)
				{
					//DumpDebug(FW_DNLD, "DL_hwIndicatePacket:  MSG_RUN_RESP - ACK\r");
...
u_long hwProcessPrivateCmd(MINIPORT_ADAPTER *pAdapter, void *Buffer)
{
	...
	switch(cmd->Code)
	{
		...
		{			
			...
			if(pAdapter->WibroStatus == WIBRO_STATE_NORMAL)
			{				
...
//		DumpDebug(RX_DPC,"Total length of packet = %lu", Length); 
	...
	if (!Adapter->DownloadMode) {
			if ( hdr->Length > MINIPORT_MAX_TOTAL_SIZE ||  ((hdr->Length + sizeof(HW_PACKET_HEADER)) > rlen) ) { 
				DumpDebug(RX_DPC,"WiBro USB: Packet length is too big (%d)", hdr->Length); 
				// skip rest of packets
				return 0;
			}
	...
	switch(hdr->Id1){
		...
		if(!Adapter->DownloadMode || PacketType == HwPktTypeData)
		{
#ifdef HARDWARE_USE_ALIGN_HEADER
			ofs += 2;		
			rlen -= 2;
	...
	// copy ethernet header if it's a data packet
		if(!Adapter->DownloadMode || PacketType == HwPktTypeData)
		{
			machdrlen = (MINIPORT_LENGTH_OF_ADDRESS * 2);
			memcpy((unsigned char *)Adapter->hw.ReceiveBuffer, Adapter->hw.EthHeader, machdrlen);
			memcpy((unsigned char *)Adapter->hw.ReceiveBuffer + machdrlen, ofs, hdr->Length);

			if(PacketType == HwPktTypeControl)
			{
				controlReceive(Adapter, Adapter->hw.ReceiveBuffer, hdr->Length + machdrlen);
			}
			else 
			{
				if (Adapter->rx_skb == NULL)
				...
				memcpy(skb_put(Adapter->rx_skb, (hdr->Length + machdrlen)), Adapter->hw.ReceiveBuffer, (hdr->Length + machdrlen));
				...
				Adapter->stats.rx.Data++;
				Adapter->netstats.rx_packets++;
				Adapter->netstats.rx_bytes += (hdr->Length + machdrlen);
